...
export class Init1759069003570 implements MigrationInterface {
    name = 'Init1759069003570'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`servico\` (\`id\` int NOT NULL AUTO_INCREMENT, \`nome\` varchar(255) NOT NULL, \`descricao\` varchar(255) NOT NULL, \`preco\` decimal(10,2) NOT NULL, \`disponivel\` tinyint NOT NULL DEFAULT 1, \`categoria\` varchar(255) NOT NULL, \`duracao\` int NULL, \`imagem\` varchar(255) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`CREATE TABLE \`carrinho\` (\`id\` int NOT NULL AUTO_INCREMENT, \`clienteId\` int NOT NULL, \`servicoId\` int NOT NULL, \`quantidade\` int NOT NULL DEFAULT '1', \`precoUnitario\` decimal(10,2) NOT NULL, \`precoTotal\` decimal(10,2) NOT NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`carrinho\` ADD CONSTRAINT \`FK_9b4d28c550905a9b86af2b73324\` FOREIGN KEY (\`clienteId\`) REFERENCES \`cliente\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`carrinho\` ADD CONSTRAINT \`FK_1af64654fdbb29cbbe199e1f115\` FOREIGN KEY (\`servicoId\`) REFERENCES \`servico\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`carrinho\` DROP FOREIGN KEY \`FK_1af64654fdbb29cbbe199e1f115\``);
        await queryRunner.query(`ALTER TABLE \`carrinho\` DROP FOREIGN KEY \`FK_9b4d28c550905a9b86af2b73324\``);
